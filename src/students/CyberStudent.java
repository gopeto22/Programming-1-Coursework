...
public class CyberStudent implements
    Student {//creating the public class Cyber student which implements the interface Student
  ...
  public CyberStudent(
      int level) {//creating the constructor of Cs student which takes int variable called level as parameter
  ...
  public int defence(
    ...
    if (building.getAllBugs().length == 0) {//checking if the getAllBugs method contains in bugs.txt in itself
      ...
      int specialAttack = normalAttack
          * 2;//creating int variable called special attack which is equal to the normal attack
      ...
      if (counter
        ...
        if (prob.equals(level
            + 20)) {//creating if statement which says if the variable we declared of type Toolbox equals level + 20
          prob.getRandomInteger(50);//then getting random integer from 0 to 50 with this method
        }
